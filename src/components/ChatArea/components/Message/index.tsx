...
const messageVariants = cva('group relative mt-2 flex flex-col rounded-2xl text-base', {
  variants: {
    variant: {
      ...
      error: 'max-w-full self-start border border-destructive/20 bg-destructive/10 px-4 py-3',
    },
  },
  defaultVariants: {
    variant: 'user',
      ...
      components: {
        ...
        code: ({
          ...
          if (inline === true || (!className && !String(children).includes('\n'))) {
            return (
              <code
                className='select-all rounded-sm bg-zinc-200 px-1.5 font-medium font-mono text-sm dark:bg-zinc-700'
                {...props}
              >
                {children}
              </code>
